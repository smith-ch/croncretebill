...
BEGIN
    ...
    FOR product_record IN 
        ...
        AND unit_price > 0
        AND user_id IS NOT NULL  -- Only process products with valid user_id
    LOOP
    ...
    -- Log how many products were processed and how many were skipped
    RAISE NOTICE 'Migration completed. Products with valid user_id processed: %', 
        (SELECT COUNT(*) FROM products WHERE unit_price IS NOT NULL AND unit_price > 0 AND user_id IS NOT NULL);
    RAISE NOTICE 'Products skipped due to null user_id: %', 
        (SELECT COUNT(*) FROM products WHERE unit_price IS NOT NULL AND unit_price > 0 AND user_id IS NULL);
END $$;

-- Handle products with null user_id by assigning them to the first available user
DO $$
DECLARE
    default_user_id UUID;
    product_record RECORD;
BEGIN
    -- Get the first user from auth.users as default owner
    SELECT id INTO default_user_id FROM auth.users LIMIT 1;
    
    IF default_user_id IS NOT NULL THEN
        -- Update products with null user_id to use the default user
        UPDATE products 
        SET user_id = default_user_id 
        WHERE user_id IS NULL;
        
        -- Now create price entries for these products
        FOR product_record IN 
            SELECT id, user_id, unit_price, name 
            FROM products 
            WHERE unit_price IS NOT NULL 
            ...
            AND id NOT IN (SELECT DISTINCT product_id FROM product_prices)
        LOOP
            INSERT INTO product_prices (
                product_id,
                user_id,
                ...
                1,
                true,
                'Precio estándar del producto (asignado automáticamente)'
            )
            ON CONFLICT (product_id, price_name) DO NOTHING;
        END LOOP;
        
        RAISE NOTICE 'Assigned orphaned products to user: %', default_user_id;
    ELSE
        RAISE NOTICE 'No users found to assign orphaned products';
    END IF;
END $$;
