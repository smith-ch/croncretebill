...
import { WelcomeTip } from "@/components/ui/welcome-tips"
      ...
      <div className="max-w-[1600px] mx-auto space-y-3 sm:space-y-4 lg:space-y-6">
        ...
        {showEmployeeView ? (
                  ...
                  {stats.recentActivity.length > 0 ? (
                    <div className="space-y-2">
                      {stats.recentActivity.slice(0, 10).map((activity, index) => (
                        <div
                          key={activity.id}
                          className={`flex items-center gap-2 p-2 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors ${index === 0 ? 'border-l-2 border-l-emerald-500 bg-emerald-900/30' : ''
                            }`}
                        >
                          <div
                            className={`p-1.5 rounded-md flex-shrink-0 ${activity.type === "invoice"
                          ...
                          <div className="text-right flex-shrink-0">
                            <p
                              className={`font-bold text-sm ${activity.type === "expense" ? "text-red-600" : "text-emerald-600"
                                }`}
                            >
          ...
          <>
            ...
            <WelcomeTip
              pageKey="dashboard"
              message="¡Bienvenido a tu panel de control! Desde aquí puedes ver tus ingresos, gastos y actividad reciente. Usa los botones de acción rápida para crear facturas, agregar clientes o registrar gastos."
              className="mb-4"
            />
                ...
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                  ...
                  <Link href="/clients">
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-slate-800 hover:bg-emerald-900/30 text-emerald-400 border-emerald-800 hover:border-emerald-600 shadow-sm hover:shadow-md transition-all duration-300 text-xs sm:text-sm"
                    >
                  ...
                  <Link href="/products">
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-slate-800 hover:bg-purple-900/30 text-purple-400 border-purple-800 hover:border-purple-600 shadow-sm hover:shadow-md transition-all duration-300 text-xs sm:text-sm hidden sm:flex"
                    >
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        <CardTitle className="text-base sm:text-lg font-bold text-blue-300 flex items-center gap-2">
                          Meta Mensual
                          <Badge className={`${monthlyProgress >= 100 ? 'bg-emerald-500' :
                ...
                <CardContent className="relative pt-0 p-3 sm:p-6">
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <div className="bg-slate-800/50 rounded-lg p-2">
                      ...
                      <div className={`text-xs ${(showWeekly
                        ? (stats.weeklyRevenue - stats.weeklyExpenseAmount)
                        : (stats.todayRevenue - stats.todayExpenseAmount)
                      ...
                      {stats.recentActivity.length > 0 ? (
                        <div className="space-y-2">
                          {stats.recentActivity.slice(0, 8).map((activity, index) => (
                            <div
                              key={activity.id}
                              className={`flex items-center gap-2 p-2 bg-slate-800 rounded-lg ${index === 0 ? 'border-l-2 border-l-emerald-500 bg-emerald-900/30' : ''
                                }`}
                            >
                              <div
                                className={`p-1 rounded-md flex-shrink-0 ${activity.type === "invoice"
                                  ? "bg-blue-500 text-white"
                                  : activity.type === "thermal_receipt"
                                    ? "bg-green-500 text-white"
                                    : "bg-red-500 text-white"
                              ...
                              <div className="text-right flex-shrink-0">
                                <p
                                  className={`font-bold text-xs ${activity.type === "expense" ? "text-red-600" : "text-emerald-600"
                                    }`}
                                >
                                  {formatCurrency(activity.total)}
                                </p>
                        ...
                        <div className="space-y-2">
                          {stats.topClients.map((client, index) => (
                            <div key={client.name} className="flex items-center gap-2 p-2 bg-slate-800/80 rounded-lg shadow-sm">
                              <div className={`w-7 h-7 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-white font-bold text-xs sm:text-lg shadow-lg flex-shrink-0 ${index === 0 ? 'bg-gradient-to-r from-yellow-500 to-orange-500' :
                      ...
                      {stats.expensesByCategory.length > 0 ? (
                        <div className="space-y-2">
                          {stats.expensesByCategory.map((category, index) => (
                            ...
                            >
                              <div className={`w-7 h-7 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-white shadow-lg flex-shrink-0 ${index === 0 ? 'bg-gradient-to-r from-red-500 to-pink-600' :
